...
/* 
    SETTINGS PAGE/S STRUCTURE:
        FOR USERS:
            o Users will only be able to access the settings page 
              a single menu rather than the dropdown the admin
              uses. This single page sends an REQ_PAGE here to be
              processed for the page for that user.

        FOR ADMINS:
            o Admins won't send the REQ_PAGE message on clicking 
              the settings button like users will.
            o ROSTERS:
                > ...
...
if(isset($_REQUEST['message']) && !empty($_REQUEST['message'])) {
    if($_REQUEST['message'] == "REQ_PAGE") {
        ...
        $page = new page;
        //get user data
        if($priv['settings'] != "full") {
            exit(json_encode(array("success" => false, "value" = "")));
        }
        #endregion

    } else if ($_REQUEST['message'] == "S_ROS_REQ" && $_SESSION['priv']['level'] == 3) {
...
EOT;
        ...
        exit(json_encode(array('success' => true, 'value' => $data)));
        #endregion

    } else if ($_REQUEST['message'] == "S_USER_REQ" && $_SESSION['priv']['level'] == 3) {
        #region ADMIN LEVEL SETTINGS FOR ALL USERS
...
EOT;
        header('Content-type: text/html');
        exit(json_encode(array('success' => 'true', 'value' => base64_encode($UserSettings))));
    #endregion

    } else if ($_REQUEST['message'] == "S_ANA_REQ" && $_SESSION['priv']['level'] == 3) {
       #region ANALYTICS SETTINGS PAGE

       #endregion

    } else if ($_REQUEST['message'] == "S_CAL_REQ" && $_SESSION['priv']['level'] == 3) {
        #region CALENDAR SETTINGS PAGE

        #endregion

    } else if((isset($_POST['message']) && !empty($_POST['message']))) {
        #region
    if($_POST['message'] == "ADD_USER") {
